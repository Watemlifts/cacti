...
(function ($) {
  ...
  const headerSelector = '.ui-multiselect-header'
  const hasFilterClass = 'ui-multiselect-hasfilter'
  const filterClass = 'ui-multiselect-filter'
  const optgroupClass = 'ui-multiselect-optgroup'
  const groupLabelClass = 'ui-multiselect-grouplabel'
  const hiddenClass = 'ui-multiselect-excluded'
  ...
  function debounce (func, wait, immediate) {
    let timeout
    return function () {
      const context = this; const args = arguments // eslint-disable-line prefer-rest-params
  ...
  $.widget('ech.multiselectfilter', {
    ...
    */
    _create: function () {
      ...
      this.$input = $(document.createElement('input'))
        ...
        .css({
          width: (typeof opts.width === 'string')
      ...
      const $label = $(document.createElement('label')).text(opts.label).append(this.$input).addClass('ui-multiselect-filter-label')
    ...
    */
    _handler: function (e) {
      const term = this.$input[0].value.toLowerCase().replace(/^\s+|\s+$/g, '')
      const filterRule = this.options.filterRule || 'contains'
      const regex = new RegExp((filterRules[filterRule] || filterRule).replace('{{term}}', term.replace(rEscape, '\\$&')), 'i')
      const searchGroups = !!this.options.searchGroups
      const $checkboxes = this.instance.$checkboxes
      const cache = this.cache // Cached text() object

      this.$rows.toggleClass(hiddenClass, !!term)

      const filteredInputs = $checkboxes.children().map(function (x) {
        const elem = this
        var $groupItems = $(elem)
    ...
    */
    updateCache: function (alsoRefresh) {
      const cache = {} // keys are like 0, 0.1, 1, 1.0, 1.1 etc.
