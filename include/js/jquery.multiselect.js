...
(function ($, undefined) {
  ...
  function insertImage (option, span) {
    const optionImageSrc = option.getAttribute('data-image-src')
    if (optionImageSrc) {
  ...
  function parse2px (dimText, $elem, isHeight) {
    ...
    const parts = dimText.match(/([<>])?=?\s*([.\d]+)\s*([eimnptx%]*)s?/i)
    const minimax = parts[1]
  ...
  $.widget('ech.multiselect', {
    ...
    * @return {object} jQuery object for the DOM element to append to.
    ...
    _getAppendEl: function () {
      let elem = this.options.appendTo // jQuery object or selector, DOM element or null.
    ...
    _buildButton: function () {
      const wrapText = this.options.wrapText || []
    ...
    _buildHeaderHtml: function () {
      ...
      if (this.options.header.constructor == Array) {
        for (let x = 0; x < this.options.header.length; x++) {
          const linkInfoKey = this.options.header[x]
    ...
    */
    _create: function () {
      ...
      const wrapText = (options.wrapText = options.wrapText || [])
      ...
      const $checkboxes = (this.$checkboxes = $(document.createElement('ul')))
    ...
    */
    _makeOption: function (option) {
      const elSelect = this.element.get(0)
      // Determine unique ID for the label & option tags
      const id = elSelect.id || this.multiselectID
      const inputID = 'ui-multiselect-' + this.multiselectID + '-' + (option.id || id + '-option-' + this.inputIdCounter++)
      // Pick up the select type from the underlying element
      const isMultiple = elSelect.multiple
      ...
      for (const name in inputAttribs) {
        if (inputAttribs[name] !== null) {
          input.setAttribute(name, inputAttribs[name])
        }
      }
      // Clone data attributes
      const optionAttribs = option.attributes
      const len = optionAttribs.length
      for (let x = 0; x < len; x++) {
        const attribute = optionAttribs[x]
    ...
    _buildOptionList: function () {
      ...
      this.element.children().each(function () {
        ...
        if (elem.tagName.toUpperCase() === 'OPTGROUP') {
          ...
          const $collapseButton = self.options.groupsCollapsable
            ? $(document.createElement('button'))
              .attr({ title: self.linkInfo.collapse.title })
              .addClass('ui-state-default ui-corner-all ui-multiselect-collapser')
              .html(self.linkInfo.collapse.icon)
            : null
          const $optGroupLabel = $(document.createElement('a'))
            .addClass('ui-multiselect-grouplabel' +
                                      (self.options.groupsSelectable ? ' ui-multiselect-selectable' : ''))
            .html(elem.getAttribute('label'))
          const $optionGroup = $(document.createElement('ul')).append(options)
          const $optGroupItem = $(document.createElement('li'))
    ...
    */
    update: function (isDefault) {
      ...
      const options = this.options
      const selectedList = options.selectedList
    ...
    _handleButtonKeyboardNav: function (e) {
      ...
      if (!this._isOpen && !this.element[0].multiple && (e.which === 38 || e.which === 40)) {
        const $inputs = this.$inputs
        const index = $inputs.index($inputs.filter(':checked'))
        if (e.which === 38 && index) {
    ...
    _bindCheckboxEvents: function () {
      ...
      self.$checkboxes.on('click.multiselect', '.ui-multiselect-grouplabel', function (e) {
        ...
        const $this = $(this)
        const $inputs = $this.next('ul').children(':not(.ui-multiselect-excluded)').find('input').not(':disabled')
        ...
        const maxSelected = self.options.maxSelected
      ...
      // collapse button
        .on('click.multiselect', '.ui-multiselect-collapser', function (e) {
          ...
          const optgroupLabel = $parent.find('.ui-multiselect-grouplabel').first().html()
          const linkInfo = self.linkInfo
          const collapsedClass = 'ui-multiselect-collapsed'
          const isCollapsed = $parent.hasClass(collapsedClass)
      ...
      // option label
        .on('mouseenter.multiselect', 'label', function (e, param) {
          if (!this.classList.contains('ui-state-disabled')) {
            const checkboxes = self.$checkboxes[0]
            const scrollLeft = checkboxes.scrollLeft
            const scrollTop = checkboxes.scrollTop
      ...
      // Keyboard navigation of the menu
        ...
        // Reference to this checkbox / radio input
          ...
          const $tags = $element.find('option')
          const isMultiple = $element[0].multiple
          const $allInputs = self.$inputs
          const numChecked = $allInputs.filter(':checked').length
          const options = self.options
          const textFxn = self.htmlAllowedFor('options') ? 'html' : 'text'
          const optionText = $input.parent().find('span')[textFxn]()
          const maxSelected = options.maxSelected

          // bail if this input is disabled or the event is cancelled
          if (input.disabled || self._trigger('click', e, { value: val, text: optionText, checked }) === false) {
            e.preventDefault()
            return
          }
    ...
    _setButtonWidth: function (recalc) {
      ...
      let width = this._selectWidth || this._getBCRWidth(this.element)
      let buttonWidth = this.options.buttonWidth || ''
      if (/\d/.test(buttonWidth)) {
        const parsed = parse2px(buttonWidth, this.element)
        const pixels = parsed.px
    ...
    _setMenuWidth: function (recalc) {
      ...
      let width = this.options.listbox ? this._selectWidth : (this._savedButtonWidth || this._getBCRWidth(this.$button))

      let menuWidth = this.options.menuWidth || ''
      if (/\d/.test(menuWidth)) {
        const parsed = parse2px(menuWidth, this.element)
        const pixels = parsed.px
        const minimax = parsed.minimax
        width = minimax < 0 ? Math.max(width, pixels) : (minimax > 0 ? Math.min(width, pixels) : pixels)
      } else { // keywords
      ...
      const headerWidth = this.$header.outerWidth(true) + this._jqWidthFix(this.$header)
      const cbWidth = this.$checkboxes.outerWidth(true) + this._jqWidthFix(this.$checkboxes)
      this.$menu.removeClass('ui-multiselect-measure')

      const contentWidth = Math.max(this.options.wrapText.indexOf('header') > -1 ? 0 : headerWidth, cbWidth)
    ...
    _setMenuHeight: function (recalc) {
      ...
      let optionHeight = self.options.menuHeight || ''
      let useSelectSize = false
      let elSelectSize = 4

      if (/\d/.test(optionHeight)) {
        // Deduct height of header & border/padding to find height available for checkboxes.
        const $header = self.$header.filter(':visible')
        const headerHeight = $header.outerHeight(true)
        const menuBorderPaddingHt = this.$menu.outerHeight(false) - this.$menu.height()
        const cbBorderPaddingHt = this.$checkboxes.outerHeight(false) - this.$checkboxes.height()
      ...
      let overflowSetting = 'hidden'
      let itemCount = 0
      const hoverAdjust = 4 // Adjustment for hover height included here.
      let ulHeight = hoverAdjust
      let ulTop = -1
    ...
    _getBCRWidth: function (elem) {
      ...
      const domRect = elem.jquery ? elem[0].getBoundingClientRect() : elem.getBoundingClientRect()
    ...
    _traverse: function (which, start) {
      const $start = $(start)
      const moveToLast = which === 38 || which === 37

      // select the first li that isn't an optgroup label / disabled
      let $next = $start.parent()[moveToLast ? 'prevAll' : 'nextAll']('li:not(:disabled, .ui-multiselect-optgroup):visible').first()
      ...
      if (!$next.length) {
        const $container = this.$checkboxes
    ...
    _toggleState: function (prop, flag) {
      return function () {
        const state = (flag === '!') ? !this[prop] : flag
    ...
    _toggleChecked: function (flag, group, filteredInputs) {
      const self = this
      const $element = self.element
      let $inputs = (group && group.length) ? group : self.$inputs
    ...
    */
    _toggleDisabled: function (flag, groupID) {
      const disabledClass = 'ui-state-disabled' // used for styling only
      ...
      if (this.options.disableInputsOnToggle) {
        ...
        const $inputs = (typeof groupID === 'undefined') ? this.$inputs : this._multiselectOptgroupFilter(groupID).find('input')
        const msDisabledClass = 'ui-multiselect-disabled'
        if (flag) {
      ...
      const $select = (typeof groupID === 'undefined') ? this.element : this._nativeOptgroupFilter(groupID).find('option')
      $select.prop({
        disabled: flag,
        'aria-disabled': flag
      })
    ...
    open: function () {
      ...
      const $inputs = this.$inputs.filter(':checked:not(.ui-state-disabled)')
      const options = this.options
      const effect = options.openEffect
      const scrollX = window.pageXOffset
      const scrollY = window.pageYOffset
      ...
      const filter = $header.find('.ui-multiselect-filter')
      if (filter.length) {
    ...
    close: function () {
      ...
      const $menu = this.$menu
      const options = this.options
      const effect = options.closeEffect
      const $button = this.$button
    ...
    */
    flipAll: function (groupID) {
      ...
      const gotID = (typeof groupID !== 'undefined') // groupID could be 0
      const maxSelected = this.options.maxSelected
      const inputCount = this.$inputs.length
      const checkedCount = this.$inputs.filter(':checked').length
      const $filteredOptgroupInputs = gotID ? this._multiselectOptgroupFilter(groupID).find('input') : null
      const gInputCount = gotID ? $filteredOptgroupInputs.length : 0
      const gCheckedCount = gotID ? $filteredOptgroupInputs.filter(':checked').length : 0
    ...
    */
    collapseAll: function (groupID) {
      ...
      const $optgroups = (typeof groupID === 'undefined') // groupID could be 0
    ...
    */
    addOption: function (attributes, text, groupID) {
      const self = this
      const textFxn = self.htmlAllowedFor('options') ? 'html' : 'text'
      const $option = $(document.createElement('option')).attr(attributes)[textFxn](text)
      const optionNode = $option.get(0)
