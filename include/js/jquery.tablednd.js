...
// Determine if this is a touch device
  ...
  $(document).ready(function () {
    function parseStyle (css) {
      ...
      while (parts.length) { objMap[parts.shift()] = parts.shift().trim() }
  ...
  jQuery.tableDnD = {
    ...
    makeIndented: function (table) {
      ...
      if ($(table).hasClass('indtd')) { return null }
      ...
      for (w = 0; w < config.hierarchyLevel; w++) { $(rows[longestCell]).find('td:first').prepend(config.indentArtifact) }
      firstCell && $(firstCell).css({ width: firstCell.offsetWidth })
      tableStyle && $(table).css(tableStyle)

      for (w = 0; w < config.hierarchyLevel; w++) { $(rows[longestCell]).find('td:first').children(':first').remove() }
      ...
      config.hierarchyLevel &&
            $(rows).each(function () {
              ...
              indentLevel <= config.hierarchyLevel &&
                    ...
                    $(this).data('level', 0)
              for (let i = 0; i < $(this).data('level'); i++) { $(this).find('td:first').prepend(config.indentArtifact) }
            })
    ...
    updateTables: function () {
      this.each(function () {
        ...
        if (this.tableDnDConfig) { $.tableDnD.makeDraggable(this) }
      })
    },
    ...
    getPosition: function (element) {
      ...
      if (element.offsetHeight == 0) { element = element.firstChild } // a table cell
    ...
    autoScroll: function (mousePos) {
      const config = this.currentTable.tableDnDConfig
    ...
    moveHorizontal: function (moving, currentRow) {
      ...
      if (!config.hierarchyLevel ||
            ...
            !currentRow ||
            this.dragObject != currentRow) { return null }
    ...
    mousemove: function (e) {
      const dragObj = $($.tableDnD.dragObject)
      const config = $.tableDnD.currentTable.tableDnDConfig
      let currentRow
      let mousePos
      let moving
      let x
      ...
      if (!$.tableDnD.dragObject) { return false }
    ...
    findDragDirection: function (x, y) {
      const sensitivity = this.currentTable.tableDnDConfig.sensitivity
      const oldX = this.oldX
      const oldY = this.oldY
      const xMin = oldX - sensitivity
      const xMax = oldX + sensitivity
      const yMin = oldY - sensitivity
      const yMax = oldY + sensitivity
      const moving = {
      ...
      if (moving.horizontal != 0) { this.oldX = x }
      if (moving.vertical != 0) { this.oldY = y }
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        // If it's the same as the current row, ignore it
        {
          if (draggedRow.is(row) ||
                    ...
                    $(row).hasClass('nodrop')) { return null } else { return row }
        }
      }
      return null
    },
    processMouseup: function () {
      ...
      const config = this.currentTable.tableDnDConfig
      const droppedRow = this.dragObject
      let parentLevel = 0
      let myLevel = 0
    ...
    serializeTable: function (table) {
      let result = ''
      const paramName = table.tableDnDConfig.serializeParamName || table.id
      const rows = table.rows
      for (let i = 0; i < rows.length; i++) {
      ...
      for (let i = 0; i < rows.length; i++) {
        if (config.hierarchyLevel) {
          ...
          if (indentLevel == 0) {
            ...
            for (let h = 0; h < previousIDs.length; h++) {
              if (previousIDs[h][1] == indentLevel) { currentID = previousIDs[h][0] }
              if (previousIDs[h][1] >= currentLevel) { previousIDs[h][1] = 0 }
            }
          }
          currentLevel = indentLevel

          if (!$.isArray(data[currentID])) { data[currentID] = [] }
