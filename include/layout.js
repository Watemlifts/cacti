...
let myRefresh
let userMenuTimer
let userMenuOpenTimer = null
let graphMenuTimer
const graphMenuElement = 0
let pulsating = true
const pageLoaded = false
let shiftPressed = false
let sessionMessage = null
let sessionMessageOpen = null
...
let popFired = false
const hostInfoHeight = 0
let menuHideResponsive = null
let marginLeftTree = null
let marginLeftConsole = null
const minTreeWidth = null
const maxTreeWidth = null
let pageName
const columnsHidden = 0
const lastColumnsHidden = {}
const lastWidth = {}
...
function basename (path, suffix) {
  ...
  if (b.indexOf('?') > 0) {
    const questionPosition = b.indexOf('?')
...
$.fn.textWidth = function (text) {
  const org = $(this)
  const html = $('<span style="display:none;white-space:nowrap;position:absolute;width:auto;left:-9999px">' + (text || org.text()) + '</span>')
  if (!text) {
...
$.fn.textBoxWidth = function () {
  const org = $(this)
  const html = $('<span style="display:none;white-space:nowrap;position:absolute;width:auto;left:-9999px">' + org.text() + '</span>')
  html.css('font-family', org.css('font-family'))
  html.css('font-weight', org.css('font-weight'))
  html.css('font-size', org.css('font-size'))
  html.css('padding', org.css('padding'))
...
$.fn.serializeObject = function () {
  let arrayData, objectData, formID
  arrayData = this.serializeArray()
  formID = $(this).attr('id')
...
function updateCheckboxes (checkboxes, clicked_element) {
  const prev_checkbox = clicked_element.closest('table').find('[data-prev-check]:checkbox')
  if (!prev_checkbox.length) {
    return
  }
  const check = prev_checkbox.attr('data-prev-check') == 'true'
  const start = checkboxes.index(prev_checkbox)
  const stop = checkboxes.index(clicked_element)
  const i = Math.min(start, stop)
  const j = Math.max(start, stop)
  for (let k = i; k <= j; k++) {
    const tr = $(checkboxes[k]).prop('checked', check).closest('tr')
    if (check) {
...
function selectUpdateRow (event, element) {
  const checkboxes = element.closest('table').find('input[type=checkbox]:not(:disabled)')
...
function dqUpdateDeps (snmp_query_id) {
  ...
  const snmp_query_graph_id = $('#sgg_' + snmp_query_id).val()
  let removeSelectAll = false

  // Check if select all is clicked
  const allChecked = $('#all_' + snmp_query_id).is(':checked')

  // Next for Data Queries
  $('tr[id^="dqline' + snmp_query_id + '_"]').each(function (data) {
    const id = $(this).attr('id')
    const pieces = id.split('_')
    const dq = pieces[0].substr(6)
    const hash = pieces[1]
...
function selectAll (attrib, checked) {
  if (attrib == 'chk') {
    ...
    const attribSplit = attrib.split('_')
    const dq = attribSplit[1]
...
function applySkin () {
  ...
  if (typeof CsrfMagic !== 'undefined') { CsrfMagic.end() }
  ...
  const showPage = $('#main').map(function (i, el) {
    const dfd = $.Deferred()
    $(el).show(function () {
      dfd.resolve()
    })
  ...
  if ($('select#user_language').hasClass('select2-hidden-accessible')) {
    ...
    $('select#user_language').select2({
      ...
      change: function () {
        ...
        const page = basename(location.pathname)
...
function displayMessages () {
  ...
  if (typeof sessionMessage.level !== 'undefined') {
    ...
    const returnStr = '<div id="messageContainer" style="display:none">' +
			'<h4>' + header + '</h4>' +
...
function sessionMessageCountdown (time) {
  let sessionMessageTimeLeft = (time / 1000)
...
function setupResponsiveMenuAndTabs () {
  $('.maintabs a.lefttab, .dropdownMenu a, .menuoptions a, #gtabs a.righttab').not('[href^="http"], [href^="https"], [href^="#"], [target="_blank"]').off('click').on('click', function (event) {
    if (!shouldCaptureClick(event)) { return }
...
function responsiveResizeGraphs () {
  const mainWidth = getMainWidth() - 30
  let myColumns = $('#columns').val()
  const isThumb = $('#thumbnails').is(':checked')
  let graphRow = $('.tableRowGraph:first').width()
  const drillDown = $('.graphDrillDown:first').outerWidth() + 15
  ...
  const myWidth = parseInt((graphRow - (drillDown * myColumns)) / myColumns)

  $('.graphimage').each(function () {
    let graph_id = $(this).attr('graph_id')
    ...
    const rra_id = $(this).attr('rra_id')
    const type = $(this).attr('graph_type')

    const original_cwidth = $('#wrapper_' + graph_id).attr('graph_width')
    const original_cheight = $('#wrapper_' + graph_id).attr('graph_height')

    /* original image attributes */
    const image_width = $(this).attr('image_width')
    const image_height = $(this).attr('image_height')
    const canvas_top = $(this).attr('canvas_top')
    const canvas_left = $(this).attr('canvas_left')
    const canvas_width = $(this).attr('canvas_width')
    const canvas_height = $(this).attr('canvas_height')

    let remove_whcss = false
    let ratio = myWidth / image_width
    ...
    const new_image_width = parseInt(image_width * ratio)
    const new_image_height = parseInt(image_height * ratio)
    const new_canvas_width = parseInt(canvas_width * ratio)
    const new_canvas_height = parseInt(canvas_height * ratio)
    const new_canvas_graph_top = parseInt(canvas_top * ratio)
    const new_canvas_graph_left = parseInt(canvas_left * ratio)
...
function tuneTables () {
  ...
  $('.cactiTable').each(function () {
    $(this).find('th:first-child').each(function () {
      const object = $(this).closest('.cactiTable')
...
function tuneTable (object, width) {
  const rows = $(object).find('tr').length
  var width = width
  const tableWidth = $(object).width()
  const totalCols = $(object).find('th').length
  const reducedWidth = 0
  const columnsHidden = countHiddenCols(object)
  const visibleColumns = totalCols - columnsHidden
  const id = $(object).attr('id')
  ...
  if (width < tableWidth) {
    ...
    const calculatedPadding = 15

    const tableHeaders = $(object).find('th')
    const tableCheckBox = $(tableHeaders).each(function () {
    ...
    $($(object).find('th').get()).each(function () {
      const isLastCheckBox = $(this).hasClass('tableSubHeaderCheckbox') && $(this).index() == tableHeaders.length - 1

      if ($(this).hasClass('noHide') || isLastCheckBox) {
        const columnWidth = $.textMetrics(this).width
...
function tuneFilter (object, width) {
  ...
  let minTds = 2
  let visTds = $(object).find('td:visible').length
...
function menuHide (store) {
  ...
  const page = basename(location.pathname).replace('.php', '')

  let myClass = ''
  const curMargin = parseInt($('#navigation_right').css('margin-left'))
...
function menuShow () {
  ...
  if ($('.cactiTreeNavigationArea').length) {
    ...
    const treeWidth = $('.cactiTreeNavigationArea').width()
...
function loadTopTab (href, id, force) {
  const stack = '' // getStackTrace(); // new Error().stack;
  console.warn('Function loadTopTab is being called\n' + stack)
  const url = href + (href.indexOf('?') > 0 ? '&' : '?') + 'headercontent=true'
...
function loadTopTabEnd (options) {
  ...
  if (options.tabId) {
    const tabElementId = '#' + options.tabId
    ...
    if (tabElement.hasClass('lefttab')) {
      ...
      const hideTabId = tabElementId.substring(0, tabElementId.length - 9)
      if (hideTabId) {
...
function setNavigationScroll () {
  ...
  $('.cactiConsoleNavigationArea, .cactiTreeNavigationArea').unbind('mousemove').on('mousemove', function (pos) {
    ...
    if (object != '') {
      const mpos = $(object).position()
      const width = $(object).outerWidth()
      const height = $(object).outerHeight()
...
function getStackTrace (levels) {
  ...
  if (levels) {
    return callstack.slice(0, levels).join('\n')
  } else {
...
function sanitizeAjaxOptions (check) {
  ...
  if (typeof check.url === 'string') {
    ...
    const hrefParts = options.url.split('?')
...
function handleAjaxResponse (html, options) {
  ...
  if (options.handle && options.redirect.trim() == '') {
    ...
    if (matches != null) {
      const htmlTitle = matches[1]
      const breadCrumbs = findElement(htmlObject, '#breadcrumbs').html()
    ...
    let scrollTop = (isMobile.any() != null) ? 1 : 0
...
function getPresentHTTPError (data) {
  if (typeof data !== 'undefined') {
    ...
    if (typeof data.responseText !== 'undefined') {
      ...
      const title_match = dataText.match(/<title>(.*?)<\/title>/)
      const head_match = dataText.match(/<h1>(.*?)<\/h1>/)
      const para_match = dataText.match(/<p>(.*?)<\/p>/)
      ...
      const returnStr = '<div id="httperror" style="display:none">' +
...
function ajaxAnchors () {
  ...
  $('a.pic, a.linkOverDark, a.linkEditMain, a.console, a.hyperLink, a.lefttab').not('[href^="http"], [href^="https"], [href^="#"], [href^="mailto"], [target="_blank"]').off('click').on('click', function (event) {
    if (!shouldCaptureClick(event)) { return }
...
function checkFormStatus (href, type, scroll_or_id) {
  ...
  if ($('.cactiFormStart').not('#chk').length) {
    $('.cactiFormStart').not('#chk').each(function () {
      ...
      const submitData = $(this).serializeForm()

      if (typeof formArray !== 'undefined' && typeof formArray[formID] !== 'undefined') {
        const formData = formArray[formID]
...
function setupSortable () {
  $('th.sortable').on('click', function (e) {
    ...
    if (!$target.is('.ui-resizable-handle')) {
      const page = $(this).find('.sortinfo').attr('sort-page')
      const column = $(this).find('.sortinfo').attr('sort-column')
      const direction = $(this).find('.sortinfo').attr('sort-direction')
      const returnto = $(this).find('.sortinfo').attr('sort-return')
...
function applyTableSizing () {
  ...
  $('.tableHeader').not('.tableFixed').find('th').resizable({
    ...
    resize: function (event, ui) {
      const resizeDelta = ui.size.width - originalSize
      const newColWidth = colWidth + resizeDelta
      const nextWidth = $(ui.element).next().attr('resizeWidth')
...
function setupEllipsis () {
  ...
  $('.maintabs-submenu, .usertabs-submenu, .submenu-ellipsis').off('click').on('click', function (event) {
    ...
    const submenu_index = $(this).attr('id').replace('menu-', 'submenu-')
    const submenu = $('#' + submenu_index)
...
function keepWindowSize () {
  $(window).resize(function (event) {
    waitForFinalEvent(function () {
      ...
      const ellipsisWidth = $('.maintabs-submenu-ellipsis').outerWidth()
      const tabHeight = $('#tabs').outerHeight()
      let mainTabPos = false
...
function hideCurrentTab (id, shrinking) {
  if ($('#' + id + '-ellipsis').length == 0) {
    const myid = id + '-ellipsis'
    const href = $('#' + id).attr('href')
    const selected = $('#' + id).hasClass('selected')
    const text = $('#' + id).text()
...
function tabsWrapping () {
  const mainTabPos = $('.maintabs:first').position()
  ...
  const ellipsisWidth = $('.maintabs-submenu-ellipsis').outerWidth()
  const mtabsWidth = $('.maintabs:not(.usertabs)').outerWidth()
...
const graphPage = urlPath + 'graph_view.php'
const pageAction = 'preview'
...
function clearGraphFilter () {
  const href = correctUrlParameters(graphPage + '?action=' + pageAction + '&clear=1')
  loadUrl({ url: href })
}
...
function saveGraphFilter (section) {
  const href = graphPage + '?action=save' +
		'&columns=' + $('#columns').val() +
		'&graphs=' + $('#graphs').val() +
...
function applyGraphFilter () {
  const href = correctUrlParameters(graphPage + '?action=' + pageAction +
  ...
  if (!options.noState) {
    if (statePushed == false) {
      if (typeof window.history.pushState !== 'undefined') {
        const myObject = { Page: myTitle, Url: cleanUrlParameters(options.url), Options: Object.assign({}, options) }
...
function removeSpikesStdDev (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=stddev&local_graph_id=' + local_graph_id
...
function removeSpikesVariance (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=variance&local_graph_id=' + local_graph_id
...
function removeSpikesInRange (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=fill&local_graph_id=' + local_graph_id + '&outlier-start=' + graph_start + '&outlier-end=' + graph_end
...
function removeRangeFill (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=float&local_graph_id=' + local_graph_id + '&outlier-start=' + graph_start + '&outlier-end=' + graph_end
...
function removeSpikesAbsolute (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=absolute&local_graph_id=' + local_graph_id
...
function dryRunStdDev (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=stddev&dryrun=true&local_graph_id=' + local_graph_id
...
function dryRunVariance (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=variance&dryrun=true&local_graph_id=' + local_graph_id
...
function dryRunSpikesInRange (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=fill&dryrun=true&local_graph_id=' + local_graph_id + '&outlier-start=' + graph_start + '&outlier-end=' + graph_end
...
function dryRunRangeFill (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=float&dryrun=true&local_graph_id=' + local_graph_id + '&outlier-start=' + graph_start + '&outlier-end=' + graph_end
...
function dryRunAbsolute (local_graph_id) {
  const strURL = urlPath + 'spikekill.php?method=absolute&dryrun=true&local_graph_id=' + local_graph_id
...
function redrawGraph (graph_id) {
  const mainWidth = getMainWidth() - 30
  const isThumb = $('#thumbnails').is(':checked')
  const myColumns = $('#columns').val()
  let graphRow = $('.tableRowGraph').width()
  const drillDown = $('.graphDrillDown:first').outerWidth() + 10
  ...
  const myWidth = (graphRow - (drillDown * myColumns)) / myColumns

  const graph_height = $('#wrapper_' + graph_id).attr('graph_height')
  const graph_width = $('#wrapper_' + graph_id).attr('graph_width')
...
function initializeGraphs (disable_cache) {
  disable_cache = disable_cache !== 'undefined'

  $.ajaxQ.abortAll()

  $('a[id$="_mrtg"]').each(function () {
    const graph_id = $(this).attr('id').replace('graph_', '').replace('_mrtg', '')
  ...
  const timestampDate1 = getTimestampFromDate($('#date1').val())
  const timestampDate2 = getTimestampFromDate($('#date2').val())

  $('a[id$="_csv"]').each(function () {
    const graph_id = $(this).attr('id').replace('graph_', '').replace('_csv', '')
  ...
  const myColumns = $('#columns').val()
  const isThumb = $('#thumbnails').is(':checked')
  const myWidth = (mainWidth - (30 * myColumns)) / myColumns
  let numGraphs = $('.graphWrapper').length
  ...
  $('.graphWrapper').each(function () {
    ...
    let rra_id = $(this).attr('rra_id')
    if (!(rra_id > 0)) {
      rra_id = 0
    }

    const graph_height = $(this).attr('graph_height')
    const graph_width = $(this).attr('graph_width')
    ...
    $.getJSON(urlPath + 'graph_json.php?rra_id=' + rra_id +
      ...
      .done(function (data) {
        ...
        let wrapper_id = '#wrapper_' + data.local_graph_id
        if (rra_id > 0) {
        ...
        let graph_id = '#graph_' + data.local_graph_id
        if (rra_id > 0) {
  ...
  $('a[id$="_util"]').each(function () {
    const graph_id = $(this).attr('id').replace('graph_', '').replace('_util', '')
    ...
    $(this).off('click').on('click', function (event) {
      ...
      const graph_id = $(this).attr('id').replace('graph_', '').replace('_util', '')
      const graph_url = urlPath + 'graph.php?action=zoom&local_graph_id=' + graph_id + '&rra_id=0&graph_start=' + getTimestampFromDate($('#date1').val()) + '&graph_end=' + getTimestampFromDate($('#date2').val())
  ...
  $('a[id$="_realtime"]').each(function () {
    ...
    $(this).off('click').on('click', function (event) {
      ...
      const graph_id = $(this).attr('id').replace('graph_', '').replace('_realtime', '')
...
function formatLanguageSelect (state) {
  ...
  const li = $('<li>')
  const wrapper = $('<div>', { text: state.text })
...
function formatColorSelect2 (state, li) {
  ...
  const regExp = /\(([^)]+)\)/
  const hex = regExp.exec(label)
  const mylabel = $($.parseHTML(label))
...
function expandClipboardSection (section) {
  ...
  const children = section.find('table').each(function (i) {
    expandClipboardSection($(this))
  })
}

function copyToClipboard (containerId) {
  const clipboardDataId = containerId.replace('copyToClipboard', 'clipboardData')
  const clipboardData = document.getElementById(clipboardDataId)
  ...
  if (clipboardData == null) {
    ...
    const clipboardHeaderId = containerId.replace('copyToClipboard', 'clipboardHeader')
    const clipboardHeader = document.getElementById(clipboardHeaderId)
    ...
    const successMessage = (!success ? clipboardNotUpdated : clipboardUpdated)
...
let snmp_password = ''
let snmp_auth_protocol = ''
let snmp_priv_protocol = ''
let snmp_priv_passphrase = ''
let snmp_security_initialized = false
...
function setSNMPSecurity () {
  if ($('#snmp_version').val() == '3') {
    if (!snmp_security_initialized) {
      ...
      const selectmenu = ($('#snmp_security_level').hasClass('select2-hidden-accessible'))
      if (selectmenu) {
...
function setSNMP () {
  const snmp_version = $('#snmp_version').val()
