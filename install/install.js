  ...
  if (StepData.Theme != 'classic') {
    ...
    $.widget('custom.iconselectmenu', $.ui.selectmenu, {
      _renderItem: function (ul, item) {
        const li = $('<li>'); const wrapper = $('<div>', { text: item.label })
        if (item.disabled) {
...
function setProgressBar (current, total, element, updatetime, fnStatus) {
  const progressBarWidth = element.width() * (current / total)
  if (fnStatus != null) {
...
function performTestConnection () {
  ...
  $.post(url, installJson)
    .done(function (data) {
      ...
      let isSuccessful = false; let statusText = testFailed
